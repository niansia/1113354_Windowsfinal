import { $i as OrthographicCamera, Bs as Vector2, Bt as Data3DTexture, Da as RGBAFormat, Ea as RGBADepthPacking, Eo as SRGBColorSpace, Fr as LinearFilter, Hs as Vector4, Ht as DataTexture, J as BufferAttribute, Kr as LoadingManager, L as BasicDepthPacking, Ns as UnsignedInt248Type, Oo as Scene, Os as Uniform, Sn as FloatType, Vr as LinearSRGBColorSpace, Vs as Vector3, Wr as Loader, Xt as DepthTexture, Y as BufferGeometry, Ys as WebGLRenderTarget, Yt as DepthStencilFormat, Zr as Material, ai as Mesh, ct as Color, di as MeshNormalMaterial, ds as Texture, gn as EventDispatcher, js as UnsignedByteType, ko as ShaderMaterial, ot as ClampToEdgeWrapping, po as RedFormat, ra as PerspectiveCamera, ri as Matrix4, si as MeshDepthMaterial, so as RGFormat, tt as CanvasTexture, vo as RepeatWrapping, xi as NearestFilter, yn as FileLoader, zr as LinearMipmapLinearFilter } from "./three.module-DTFgBgFZ.js";
...
function getCoefficients(n) {
	let result;
	if (n === 0) result = new Float64Array(0);
	else if (n === 1) result = new Float64Array([1]);
	else if (n > 1) {
		let row0 = new Float64Array(n);
		let row1 = new Float64Array(n);
		for (let y = 1; y <= n; ++y) {
			for (let x = 0; x < y; ++x) row1[x] = x === 0 || x === y - 1 ? 1 : row0[x - 1] + row0[x];
			result = row1;
			row1 = row0;
			row0 = result;
		}
...
var GaussKernel = class {
	...
	*/
	generate(kernelSize, edgeBias) {
		if (kernelSize < 3 || kernelSize > 1020) throw new Error("The kernel size must be in the range [3, 1020]");
		...
		for (let i = 0, l = weights.length, s = 1 / sum; i < l; ++i) weights[i] *= s;
		const linearWeightSum = (linearWeights.reduce((a, b) => a + b, 0) - linearWeights[0] * .5) * 2;
		if (linearWeightSum !== 0) for (let i = 0, l = linearWeights.length, s = 1 / linearWeightSum; i < l; ++i) linearWeights[i] *= s;
...
var ASCIITexture = class extends CanvasTexture {
	...
	*/
	constructor({ characters = " .:,'-^=*+?!|0#X%WM@", font = "Arial", fontSize = 54, size = 1024, cellCount = 16 } = {}) {
		super(document.createElement("canvas"), void 0, RepeatWrapping, RepeatWrapping);
...
var ASCIIEffect = class extends Effect {
	...
	*/
	constructor({ asciiTexture = new ASCIITexture(), cellSize = 16, color: color2 = null, inverted = false } = {}) {
		super("ASCIIEffect", ascii_default, { uniforms: /* @__PURE__ */ new Map([
	...
	*/
	...
	set asciiTexture(value) {
		...
		if (currentTexture !== null && currentTexture !== value) currentTexture.dispose();
		if (value !== null) {
	...
	*/
	dispose() {
		if (this.asciiTexture !== null) this.asciiTexture.dispose();
		super.dispose();
	}
};
var KernelSize = {
	...
	*/
	constructor(colorOutput = false, luminanceRange = null) {
		super({
			...
			defines: { THREE_REVISION: "184".replace(/\D+/g, "") },
			uniforms: {
				inputBuffer: new Uniform(null),
...
var BokehEffect = class extends Effect {
	...
	*/
	constructor({ blendFunction, focus = .5, dof = .02, aperture = .015, maxBlur = 1 } = {}) {
	...
	*/
	constructor({ blendFunction = BlendFunction.SRC, gamma = 2 } = {}) {
		super("GammaCorrectionEffect", gamma_correction_default, {
			blendFunction,
			uniforms: /* @__PURE__ */ new Map([["gamma", new Uniform(gamma)]])
		});
	}
};
var GlitchMode = {
...
var LensDistortionEffect = class extends Effect {
	...
	*/
	constructor({ distortion = new Vector2(0, 0), principalPoint = new Vector2(0, 0), focalLength = new Vector2(1, 1), skew = 0 } = {}) {
		super("LensDistortionEffect", lens_distortion_default, { uniforms: /* @__PURE__ */ new Map([
...
var LUT1DEffect = class extends Effect {
	...
	*/
	...
	set lut(value) {
		...
		if (value !== null && (value.type === 1015 || value.type === 1016)) this.defines.set("LUT_PRECISION_HIGH", "1");
	}
};
...
var RealisticBokehEffect = class extends Effect {
	...
	*/
	constructor({ blendFunction, focus = 1, focalLength = 24, fStop = .9, luminanceThreshold = .5, luminanceGain = 2, bias = .5, fringe = .7, maxBlur = 1, rings = 3, samples = 2, showFocus = false, manualDoF = false, pentagon = false } = {}) {
	...
	*/
	...
	set showFocus(value) {
		if (this.showFocus !== value) {
			if (value) this.defines.set("SHOW_FOCUS", "1");
			else this.defines.delete("SHOW_FOCUS");
			this.setChanged();
		}
	}
	/**
	...
	set manualDoF(value) {
		if (this.manualDoF !== value) {
			if (value) {
				...
				this.uniforms.get("dof").value = new Vector4(.2, 1, .2, 2);
			} else {
	...
	*/
	...
	set pentagon(value) {
		if (this.pentagon !== value) {
			if (value) this.defines.set("PENTAGON", "1");
			else this.defines.delete("PENTAGON");
			this.setChanged();
		}
	}
};
	...
	*/
	constructor(texelSize = new Vector2(), mode = EdgeDetectionMode.COLOR) {
		super({
			...
			defines: {
				THREE_REVISION: "184".replace(/\D+/g, ""),
...
var ToneMappingEffect = class extends Effect {
	...
	*/
	...
	set mode(value) {
		...
		const cineonToneMapping = "184".replace(/\D+/g, "") >= 168 ? "CineonToneMapping(texel)" : "OptimizedCineonToneMapping(texel)";
		this.defines.clear();
...
var LUT3dlLoader = class extends Loader {
	...
	*/
	parse(input) {
		...
		if (result === null) throw new Error("Missing grid information");
		const gridLines = result[0].trim().split(/\s+/g).map((n) => Number(n));
		const gridStep = gridLines[1] - gridLines[0];
		const size = gridLines.length;
		const sizeSq = size ** 2;
		for (let i = 1, l = gridLines.length; i < l; ++i) if (gridStep !== gridLines[i] - gridLines[i - 1]) throw new Error("Inconsistent grid size");
		const data = new Float32Array(size ** 3 * 4);
		let maxValue = 0;
		let index = 0;
		...
		const maxBitValue = Math.pow(2, Math.ceil(Math.log2(maxValue)));
...
var LUTCubeLoader = class extends Loader {
	...
	*/
	parse(input) {
		...
		if (result === null) throw new Error("Missing LUT_3D_SIZE information");
		const size = Number(result[1]);
		const data = new Float32Array(size ** 3 * 4);
		const domainMin = new Vector3(0, 0, 0);
		const domainMax = new Vector3(1, 1, 1);
		result = regExpDomainMin.exec(input);
		if (result !== null) domainMin.set(Number(result[1]), Number(result[2]), Number(result[3]));
		result = regExpDomainMax.exec(input);
		if (result !== null) domainMax.set(Number(result[1]), Number(result[2]), Number(result[3]));
...
var BoxBlurMaterial = class extends ShaderMaterial {
	...
	*/
	...
	set kernelSize(value) {
		if (value % 2 === 0) throw new Error("The kernel size must be an odd number");
	...
	*/
	...
	set bilateral(value) {
		if (value !== null) this.defines.BILATERAL = "1";
		else delete this.defines.BILATERAL;
		this.needsUpdate = true;
	}
	/**
...
var BoxBlurPass = class extends Pass {
	...
	*/
	initialize(renderer, alpha, frameBufferType) {
		if (renderer !== null) this.blurMaterial.maxVaryingVectors = renderer.capabilities.maxVaryings;
		if (frameBufferType !== void 0) {
			this.renderTargetA.texture.type = frameBufferType;
			this.renderTargetB.texture.type = frameBufferType;
			if (frameBufferType !== 1009) this.fullscreenMaterial.defines.FRAMEBUFFER_PRECISION_HIGH = "1";
...
var threeRevision = Number("184".replace(/\D+/g, ""));
...
var DepthPickingPass = class extends DepthCopyPass {
	...
	*/
	constructor({ depthPacking = RGBADepthPacking, mode = DepthCopyMode.SINGLE } = {}) {
		...
		this.pixelBuffer = depthPacking === 3201 ? new Uint8Array(4) : new Float32Array(4);
		this.callback = null;
	}
	/**
	...
	*/
	render(renderer, inputBuffer, outputBuffer, deltaTime, stencilTest) {
		...
		for (let i = 0, l = Math.max(this.iterations, 1); i < l; ++i) {
			...
			renderer.render(scene, camera);
			if (i === 0 && l > 1) previousBuffer = renderTargetB;
		}
		this.fullscreenMaterial = this.copyMaterial;
		renderer.setRenderTarget(this.renderToScreen ? null : outputBuffer);
		renderer.render(scene, camera);
...
function calculateTetrahedronVolume(a, b, c2, d) {
	...
	return Math.abs(dot + constant) * triangleArea / 3;
}
...
function tetrahedralSample(data, size, u, v4, w, color2) {
	...
	const sv = py - minY;
	const sw = pz - minZ;
	if (minX === px && minY === py && minZ === pz) sample(data, size, px, py, pz, color2);
	else {
		let vertices;
		if (su >= sv && sv >= sw) vertices = T[0];
		else if (su >= sw && sw >= sv) vertices = T[1];
		else if (sw >= su && su >= sv) vertices = T[2];
		else if (sv >= su && su >= sw) vertices = T[3];
		else if (sv >= sw && sw >= su) vertices = T[4];
		else if (sw >= sv && sv >= su) vertices = T[5];
...
function smoothArea(d) {
	const a1 = area[0];
	const a2 = area[1];
	const b1X = Math.sqrt(a1[0] * 2) * .5;
	const b1Y = Math.sqrt(a1[1] * 2) * .5;
	const b2X = Math.sqrt(a2[0] * 2) * .5;
	const b2Y = Math.sqrt(a2[1] * 2) * .5;
...
function getOrthArea(p1X, p1Y, p2X, p2Y, x, result) {
	...
	if (x1 >= p1X && x1 < p2X || x2 > p1X && x2 <= p2X) if (Math.sign(y1) === Math.sign(y2) || Math.abs(y1) < 1e-4 || Math.abs(y2) < 1e-4) {
		const a = (y1 + y2) / 2;
		if (a < 0) {
			result[0] = Math.abs(a);
			result[1] = 0;
		...
		if ((Math.abs(a1) > Math.abs(a2) ? a1 : -a2) < 0) {
...
function getOrthAreaForPattern(pattern, left, right, offset, result) {
	...
	switch (pattern) {
		case 0:
			...
			break;
		case 1:
			if (left <= right) getOrthArea(0, o2, d / 2, 0, left, result);
			else {
				result[0] = 0;
				result[1] = 0;
			}
			break;
		case 2:
			if (left >= right) getOrthArea(d / 2, 0, d, o2, left, result);
			else {
				result[0] = 0;
				result[1] = 0;
			}
		...
		case 3:
			...
			break;
		case 4:
			if (left <= right) getOrthArea(0, o1, d / 2, 0, left, result);
			else {
				result[0] = 0;
				result[1] = 0;
			}
		...
		case 6:
			...
			} else getOrthArea(0, o1, d, o2, left, result);
			break;
		case 7:
			getOrthArea(0, o1, d, o2, left, result);
			break;
		case 8:
			if (left >= right) getOrthArea(d / 2, 0, d, o1, left, result);
			else {
				result[0] = 0;
				result[1] = 0;
			}
		...
		case 9:
			if (Math.abs(offset) > 0) {
				...
				result[0] = (a1[0] + a2[0]) / 2;
				result[1] = (a1[1] + a2[1]) / 2;
			} else getOrthArea(0, o2, d, o1, left, result);
			break;
		case 10:
			result[0] = 0;
			result[1] = 0;
...
function isInsideArea(a1X, a1Y, a2X, a2Y, x, y) {
	...
	if (!result) {
		...
		const a = a2Y - a1Y;
		const b = a1X - a2X;
		result = a * (x - xm) + b * (y - ym) > 0;
	}
	return result;
}
function getDiagAreaForPixel(a1X, a1Y, a2X, a2Y, pX, pY) {
	let n = 0;
	for (let y = 0; y < DIAGONAL_SAMPLES; ++y) for (let x = 0; x < DIAGONAL_SAMPLES; ++x) {
		const offsetX = x / (DIAGONAL_SAMPLES - 1);
		const offsetY = y / (DIAGONAL_SAMPLES - 1);
		if (isInsideArea(a1X, a1Y, a2X, a2Y, pX + offsetX, pY + offsetY)) ++n;
	}
...
function generatePatterns(patterns, offset, orthogonal) {
	...
	for (let i = 0, l = patterns.length; i < l; ++i) {
		...
		for (let y = 0; y < size; ++y) for (let x = 0; x < size; ++x) {
			if (orthogonal) getOrthAreaForPattern(i, x, y, offset, result);
			else getDiagAreaForPattern(i, x, y, offset, result);
			const c2 = (y * size + x) * 2;
...
function assemble(baseX, baseY, patterns, edges2, size, orthogonal, target) {
	...
	for (let i = 0, l = patterns.length; i < l; ++i) {
		...
		for (let y = 0; y < size; ++y) for (let x = 0; x < size; ++x) {
			...
			const c2 = ((edge[1] * size + baseY + y) * dstWidth + pX) * 4;
...
var SMAAAreaImageData = class {
	...
	*/
	static generate() {
		const width = 10 * ORTHOGONAL_SIZE;
		...
		for (let i = 3, l = data.length; i < l; i += 4) data[i] = 255;
		for (let i = 0; i < 16; ++i) {
...
function generate(useCache = true) {
	const workerURL = URL.createObjectURL(new Blob([worker_default2], { type: "text/javascript" }));
	const worker = new Worker(workerURL);
	URL.revokeObjectURL(workerURL);
	return new Promise((resolve, reject) => {
		worker.addEventListener("error", (event) => reject(event.error));
		worker.addEventListener("message", (event) => {
			...
			const urls = [searchImageData.toCanvas().toDataURL("image/png", 1), areaImageData.toCanvas().toDataURL("image/png", 1)];
			if (useCache) {
...
var SMAAImageGenerator = class {
	...
	*/
	generate() {
		...
		const cachedURLs = useCache ? [localStorage.getItem("smaa-search"), localStorage.getItem("smaa-area")] : [null, null];
		return (cachedURLs[0] !== null && cachedURLs[1] !== null ? Promise.resolve(cachedURLs) : generate(useCache)).then((urls) => {
			return new Promise((resolve, reject) => {
				const searchImage = new Image();
				const areaImage = new Image();
...
function deltaLeft(left, top) {
	let d = 0;
	if (top[3] === 1) d += 1;
	if (d === 1 && top[2] === 1 && left[1] !== 1 && left[3] !== 1) d += 1;
	return d;
}
function deltaRight(left, top) {
	let d = 0;
	if (top[3] === 1 && left[1] !== 1 && left[3] !== 1) d += 1;
	if (d === 1 && top[2] === 1 && left[0] !== 1 && left[2] !== 1) d += 1;
	return d;
}
var SMAASearchImageData = class {
	...
	*/
	static generate() {
		...
		for (let i = 0, y = height - croppedHeight; y < height; ++y) for (let x = 0; x < croppedWidth; ++x, i += 4) {
...
//#endregion
export { ASCIIEffect, ASCIITexture, AdaptiveLuminanceMaterial, AdaptiveLuminancePass, BlendFunction, BlendMode, BloomEffect, KawaseBlurPass as BlurPass, KawaseBlurPass, BokehEffect, BokehMaterial, BoxBlurMaterial, BoxBlurPass, BrightnessContrastEffect, ChromaticAberrationEffect, CircleOfConfusionMaterial, ClearMaskPass, ClearPass, ColorAverageEffect, ColorChannel, ColorDepthEffect, EdgeDetectionMaterial as ColorEdgesMaterial, EdgeDetectionMaterial, KawaseBlurMaterial as ConvolutionMaterial, KawaseBlurMaterial, CopyMaterial, CopyPass, CopyPass as SavePass, DepthComparisonMaterial, DepthCopyMaterial, DepthCopyMode, DepthCopyPass, DepthCopyPass as DepthSavePass, DepthDownsamplingMaterial, DepthDownsamplingPass, DepthEffect, DepthMaskMaterial, DepthOfFieldEffect, DepthPass, DepthPickingPass, DepthTestStrategy, Disposable, DotScreenEffect, DownsamplingMaterial, EdgeDetectionMode, Effect, EffectAttribute, EffectComposer, EffectMaterial, EffectPass, EffectShaderData, EffectShaderSection, EffectShaderSection as Section, FXAAEffect, GammaCorrectionEffect, GaussKernel, GaussianBlurMaterial, GaussianBlurPass, GlitchEffect, GlitchMode, GodRaysEffect, GodRaysMaterial, GridEffect, HueSaturationEffect, ImmutableTimer, Initializable, KernelSize, LUT1DEffect, LUT3DEffect, LUT3DEffect as LUTEffect, LUT3dlLoader, LUTCubeLoader, LUTOperation, LambdaPass, LensDistortionEffect, LookupTexture, LookupTexture as LookupTexture3D, LuminanceMaterial, LuminancePass, MaskFunction, MaskMaterial, MaskPass, MipmapBlurPass, NoiseEffect, NoiseTexture, NormalPass, OutlineMaterial as OutlineEdgesMaterial, OutlineMaterial, OutlineEffect, OverrideMaterialManager, Pass, PixelationEffect, PredicationMode, RawImageData, RealisticBokehEffect, RenderPass, Resizable, Resolution as Resizer, Resolution, SMAAAreaImageData, SMAAEffect, SMAAImageGenerator, SMAAImageLoader, SMAAPreset, SMAASearchImageData, SMAAWeightsMaterial, SSAOEffect, SSAOMaterial, ScanlineEffect, Selection, SelectiveBloomEffect, SepiaEffect, ShaderPass, ShockWaveEffect, TetrahedralUpscaler, TextureEffect, TiltShiftBlurMaterial, TiltShiftBlurPass, TiltShiftEffect, Timer, ToneMappingEffect, ToneMappingMode, UpsamplingMaterial, VignetteEffect, VignetteTechnique, WebGLExtension, version };
