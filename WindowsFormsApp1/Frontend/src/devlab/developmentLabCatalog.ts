...
export type DevGroup = 'linear' | 'trees' | 'sorting' | 'searching' | 'graph' | 'dp';
export type DevModuleKind = 'linear' | 'tree' | 'heap' | 'sort' | 'search' | 'graph' | 'dp';
...
export const DEV_GROUPS: Array<{ id: DevGroup; label: string }> = [
...
export const getDevModule = (id: DevModuleId): DevModule =>
  DEV_MODULES.find((module) => module.id === id) ?? DEV_MODULES[0];
