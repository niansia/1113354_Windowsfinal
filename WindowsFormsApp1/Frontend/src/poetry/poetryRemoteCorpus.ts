...
// Shape of the generated bundle produced by tools/gen_poetry.mjs.
interface BundlePoet {
  id: string;
  name: string;
  dynasty: string;
  bio?: string;
...
export interface PoetryBundle {
  meta: {
    generatedAt?: string;
    source?: string;
    sourceUrl?: string;
    scannedPoems?: number;
    totalAvailablePoems?: number;
    poets?: number;
    poemsWithText?: number;
    edges?: number;
  };
  poets: BundlePoet[];
  edges: BundleEdge[];
...
export interface MergedCorpus {
  ...
  poems: Poem[];
  graph: PoetryGraph;
  meta: PoetryBundle['meta'];
}

export interface PoetryDisplayStats {
  loadedPoets: number;
  loadedTextPoems: number;
  archivePoets: number;
  archivePoems: number;
  relationEdges: number;
}

export const POETRY_ARCHIVE_TOTALS = {
  poets: 32657,
  poems: 933857
} as const;

const KNOWN_DYNASTIES: PoetryDynasty[] = ['先秦', '漢', '魏晉', '南北朝', '唐', '宋', '元', '明', '清'];
const KNOWN_FORMS: PoetryForm[] = ['五絕', '七絕', '五律', '七律', '古體', '樂府', '詞', '曲', '自由'];

const asDynasty = (value: string): PoetryDynasty =>
  (KNOWN_DYNASTIES as string[]).includes(value) ? (value as PoetryDynasty) : '唐';
const asForm = (value: string): PoetryForm =>
  (KNOWN_FORMS as string[]).includes(value) ? (value as PoetryForm) : '古體';
 ...
 */
export async function fetchPoetryBundle(
  signal?: AbortSignal
): Promise<PoetryBundle | null> {
  try {
    ...
    const data = (await res.json()) as PoetryBundle;
    if (!Array.isArray(data?.poets) || !Array.isArray(data?.poems)) return null;
    return data;
  } catch {
    return null;
  }
...
export function mergeCorpus(
  curatedPoets: Poet[],
  curatedPoems: Poem[],
  bundle: PoetryBundle | null
): MergedCorpus {
  const poetById = new Map<string, Poet>();
  const curatedById = new Map<string, Poet>();
  for (const poet of curatedPoets) {
    const clone: Poet = { ...poet, themes: [...poet.themes], styles: [...poet.styles] };
    poetById.set(poet.id, clone);
    curatedById.set(poet.id, clone);
  }

  const poems: Poem[] = curatedPoems.map((poem) => ({ ...poem }));
  const edgeMap = new Map<string, PoetryGraphEdge>();

  // curated explicit relations first (richest reasons)
  for (const poet of curatedPoets) {
    for (const relation of poet.relations) {
      if (!curatedById.has(relation.poetId) && !bundle) continue;
  ...
  if (bundle) {
    ...
    const featuredByPoet = new Map<string, string[]>();
    for (const bm of bundle.poems) {
      if (!poetById.has(bm.poetId)) continue;
      poems.push({
        id: bm.id,
        title: bm.title,
        poetId: bm.poetId,
      ...
      if (list.length < 3) list.push(bm.id);
      featuredByPoet.set(bm.poetId, list);
    }
    for (const [poetId, ids] of featuredByPoet) {
      const poet = poetById.get(poetId);
      if (poet && !poet.featuredPoemIds.length) poet.featuredPoemIds = ids;
    }

    for (const be of bundle.edges) {
      if (!poetById.has(be.source) || !poetById.has(be.target)) continue;
      const key = edgeKey(be.source, be.target);
      if (edgeMap.has(key)) continue; // keep curated relation if both exist
      edgeMap.set(key, {
        source: be.source,
        target: be.target,
        kind: (be.kind as PoetryGraphEdge['kind']) ?? '交遊',
  ...
  const poems_byPoet = new Map<string, string[]>();
...
export function getPoetryDisplayStats(corpus: MergedCorpus): PoetryDisplayStats {
  ...
  return {
    ...
    archivePoets: Math.max(corpus.poets.length, metaPoets, POETRY_ARCHIVE_TOTALS.poets),
    archivePoems: Math.max(corpus.poems.length, metaArchivePoems, POETRY_ARCHIVE_TOTALS.poems),
