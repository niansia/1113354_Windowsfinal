...
export function buildPoetryGraph(poets: Poet[], poems: Poem[]): PoetryGraph {
  const poemIdsByPoet = new Map<string, string[]>();
  ...
  const edges = new Map<string, PoetryGraphEdge>();

  for (const poet of poets) {
    for (const relation of poet.relations) {
      if (!poets.some((candidate) => candidate.id === relation.poetId)) continue;
      const key = edgeKey(poet.id, relation.poetId);
      const candidate: PoetryGraphEdge = {
        source: poet.id,
      ...
      if (!current || candidate.weight > current.weight) edges.set(key, candidate);
    }
  }
...
export function findPoetPath(graph: PoetryGraph, startId: string, endId: string): PoetryPath {
  if (startId === endId) return { poetIds: [startId], edges: [] };
  const adjacency = new Map<string, Array<{ next: string; edge: PoetryGraphEdge }>>();
  ...
  const previous = new Map<string, { poetId: string; edge: PoetryGraphEdge }>();
  ...
  while (queue.length) {
    ...
    for (const neighbor of adjacency.get(current) ?? []) {
      ...
      if (neighbor.next === endId) {
        const poetIds = [endId];
        const edges: PoetryGraphEdge[] = [];
        let cursor = endId;
        while (cursor !== startId) {
          const step = previous.get(cursor);
          if (!step) return { poetIds: [], edges: [] };
