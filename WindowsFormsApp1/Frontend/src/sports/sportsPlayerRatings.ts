...
const RAW_STAR_VALUE: Record<string, number> = {
  // ~96-99 — generational
...
const STAR_VALUE = new Map<string, number>();
for (const [name, value] of Object.entries(RAW_STAR_VALUE)) {
  const key = normalize(name);
  if (!STAR_VALUE.has(key)) STAR_VALUE.set(key, value);
  const parts = key.split(' ');
  if (parts.length === 2) {
    const flipped = `${parts[1]} ${parts[0]}`;
    if (!STAR_VALUE.has(flipped)) STAR_VALUE.set(flipped, value);
  }
}

// Returns a 0-100 strength/value tier for a known star, or null for everyone else.
export function starPlayerTier(name: string | undefined | null): number | null {
  const key = normalize(name ?? '');
  if (!key) return null;
