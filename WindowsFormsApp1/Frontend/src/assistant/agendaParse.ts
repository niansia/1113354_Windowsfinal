...
// Dependency-free natural-language parser for spoken reminders. Turns utterances like
// "幫我在明天下午三點標註開會" or "remind me tomorrow at 9am to call mom" into a concrete
// { date, time, title } so the assistant can drop a real event onto the calendar. Chinese
// is supported most thoroughly (the user's primary language); en/ja/ko cover the basics.

export interface AgendaParse {
  date: string;        // 'YYYY-MM-DD' (local) — defaults to today when no date is found
  time: string | null; // 'HH:mm' or null for an all-day item
  title: string;       // the task text with temporal + filler words removed
  hasDate: boolean;
  hasTime: boolean;
}
...
const mondayIndex = (dow: number) => (dow + 6) % 7; // Mon→0 … Sun→6
function weekdayDate(now: Date, targetDow: number, weekOffset: number, explicitWeek: boolean): Date {
  ...
  if (!explicitWeek && weekOffset === 0) {
    ...
    if (delta < 0) delta += 7;
    if (delta === 0) delta = 7; // "週五" said on Friday → next Friday
...
const MARKER_DEFAULT: Array<[RegExp, number]> = [
...
function findDate(text: string, now: Date): { date: Date; consumed: string[] } | null {
  // X 月 Y 日/號
  ...
  if (inDays) {
    ...
    if (n !== null) return { date: addDays(now, n), consumed: [inDays[0]] };
  }
  ...
  for (const [re, offset] of REL_DAYS) {
    ...
    if (m) return { date: addDays(now, offset), consumed: [m[0]] };
  }
...
function findTime(text: string): { hour: number; minute: number; consumed: string[] } | null {
  ...
  if (clock) {
    ...
    if (/p/.test(mer) && hour < 12) hour += 12;
    if (/a/.test(mer) && hour === 12) hour = 0;
  ...
  if (cn) {
    ...
    if (hour !== null && hour >= 0 && hour <= 24) {
      ...
      else if (tail === '整' || tail === undefined) minute = 0;
      else { const m = parseNum(tail); if (m !== null && m >= 0 && m <= 59) minute = m; }
      ...
      if (PM_MARK.test(marker) && hour < 12) hour += 12;
      else if (AM_MARK.test(marker) && hour === 12) hour = 0;
      if (hour === 24) hour = 0;
  ...
  if (enHour) {
    ...
    if (/p/.test(mer) && hour < 12) hour += 12;
    if (/a/.test(mer) && hour === 12) hour = 0;
    if (hour >= 0 && hour <= 23) return { hour, minute: 0, consumed: [enHour[0]] };
  }

  // Bare meridiem word with no explicit hour → sensible default.
  for (const [re, hour] of MARKER_DEFAULT) {
    const m = text.match(re);
    if (m) return { hour, minute: 0, consumed: [m[0]] };
  }
  return null;
}
...
function cleanTitle(raw: string, consumed: string[]): string {
  let title = raw;
  for (const piece of consumed) {
    if (piece) title = title.split(piece).join(' ');
  }
...
export function parseAgenda(raw: string, _lang: Lang, now: Date = new Date()): AgendaParse {
  const text = (raw ?? '').trim();
