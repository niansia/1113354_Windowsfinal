export type ChannelMedium = 'fiber' | 'copper' | 'wireless' | 'acoustic';
export type ReliabilityLevel = 'steady' | 'watch' | 'review';
...
export interface ProcessorRegister {
  name: string;
  value: string;
  note: string;
}
...
const MEDIUMS: Record<ChannelMedium, { label: string; speedMps: number; attenuationDbPerKm: number; baseMarginDb: number }> = {
...
const toBytes = (payload: string) => Array.from(new TextEncoder().encode(payload || 'Fusion'));
...
export const estimateChannel = (input: ChannelInput): ChannelEstimate => {
  ...
  const reliabilityLevel: ReliabilityLevel =
    signalMarginDb >= 14 ? 'steady' : signalMarginDb >= 8 ? 'watch' : 'review';
...
export const runProcessorTrace = (frame: SignalFrame): ProcessorTrace => {
