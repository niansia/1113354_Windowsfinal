import { STYLE_COLORS } from './styleCatalog.js';
import type {
  ColorRecommendation,
  StyleColorRole,
  StyleLook,
...
export const setWardrobePiece = <Slot extends WardrobeSlot>(
  look: StyleLook,
  slot: Slot,
  piece: WardrobePieceBySlot[Slot]
): StyleLook => {
...
export const recommendColors = (
  ...
  return STYLE_COLORS
    ...
    .map((color) => {
      ...
      if (contrast > 0.2) reasons.push('建立清楚但不刺眼的對比');
      else reasons.push('維持低對比的柔和層次');
