export interface CutoutResult {
  url: string;
  /** content bounding box, normalized 0..1 over the cropped output */
  aspect: number;
}
 ...
 */
export function cutoutGarment(
  img: HTMLImageElement,
  opts: { tolerance?: number; cropTopFrac?: number; maxSize?: number } = {}
): CutoutResult | null {
  ...
  sr /= sampleIdx.length; sg /= sampleIdx.length; sb /= sampleIdx.length;
  const tol2 = tolerance * tolerance * 3;
  const isBgColor = (j: number) => {
    const dr = px[j * 4] - sr;
  ...
  const pushSeed = (x: number, y: number) => {
    const i = y * w + x;
    if (!bg[i] && isBgColor(i)) { bg[i] = 1; stack.push(i); }
  };
  for (let x = 0; x < w; x += 1) { pushSeed(x, 0); pushSeed(x, h - 1); }
  for (let y = 0; y < h; y += 1) { pushSeed(0, y); pushSeed(w - 1, y); }
  ...
  while (stack.length) {
    ...
    for (let k = 0; k < 4; k += 1) {
      ...
      const j = neighbors[k];
      if (bg[j]) continue;
      if (isBgColor(j)) { bg[j] = 1; stack.push(j); }
    }
  }
  ...
  if (maxX <= minX || maxY <= minY) return { url: canvas.toDataURL('image/png'), aspect: w / h };
  ...
  if (!octx) return { url: canvas.toDataURL('image/png'), aspect: w / h };
