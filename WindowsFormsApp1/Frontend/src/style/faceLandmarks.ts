import type { FaceModel, FeatureEllipse } from './styleTypes.js';

export interface Pt { x: number; y: number }

export interface EyeGeo {
  /** upper lash line, ordered inner-corner → outer-corner */
  upper: Pt[];
  /** lower lash line, ordered inner-corner → outer-corner */
  lower: Pt[];
  inner: Pt;
  outer: Pt;
  center: Pt;
  w: number;
  h: number;
  /** vertical gap from the upper lash to the brow (normalized, positive) */
  browGap: number;
}

/** Everything the makeup renderer needs, in normalized 0..1 image coordinates. */
export interface FaceGeometry {
  source: 'mediapipe' | 'ellipse';
  lipsOuter: Pt[];
  lipsInner: Pt[];
  lipCenter: Pt;
  lipW: number;
  lipH: number;
  eyes: EyeGeo[];
  /** closed eyebrow regions (for fill tint) */
  browPolys: Pt[][];
  cheeks: Pt[];
  cheekbones: Pt[];
  noseBridge: Pt[];
...
const pick = (points: Pt[], indices: number[]) => indices.map((i) => points[i]).filter(Boolean);
const centroid = (pts: Pt[]): Pt => {
  if (!pts.length) return { x: 0.5, y: 0.5 };
  const sum = pts.reduce((acc, p) => ({ x: acc.x + p.x, y: acc.y + p.y }), { x: 0, y: 0 });
  return { x: sum.x / pts.length, y: sum.y / pts.length };
};
const span = (pts: Pt[], axis: 'x' | 'y') => {
  const values = pts.map((p) => p[axis]);
  return Math.max(...values) - Math.min(...values);
};

function eyeGeo(points: Pt[], upperIdx: number[], lowerIdx: number[], innerIdx: number, outerIdx: number, browPoly: Pt[]): EyeGeo {
...
function buildFromLandmarks(points: Pt[]): FaceGeometry {
...
const arc = (e: FeatureEllipse, startDeg: number, endDeg: number, n = 7): Pt[] => {
...
function buildFromEllipses(model: FaceModel): FaceGeometry {
  const lm = model.landmarks;
  const eyeFromEllipse = (e: FeatureEllipse): EyeGeo => {
  ...
  const browFromEye = (e: FeatureEllipse): Pt[] =>
    arc({ ...e, y: e.y - e.ry * 2.4, rx: e.rx * 1.15, ry: e.ry * 0.5 }, 0, 360, 10);
...
export async function detectFaceGeometry(
  ...
  try {
    const landmarker = (await getLandmarker()) as
      | { detect: (img: HTMLImageElement) => { faceLandmarks: Pt[][] } }
      | null;
    if (landmarker) {
