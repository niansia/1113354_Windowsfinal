import type { EyeGeo, Pt } from './faceLandmarks.js';

export interface CoverTransformInput {
  sourceWidth: number;
  sourceHeight: number;
...
export type EyelinerGeometryStyle =
  | 'natural'
  | 'tightline'
  | 'puppy'
  | 'wing'
...
export type LashGeometryStyle = 'natural' | 'doll' | 'cat' | 'wispy';

export interface EyelinerGeometryOptions {
  style: EyelinerGeometryStyle;
  intensity: number;
  thickness: number;
  wingLength: number;
  wingLift: number;
...
export interface EyelinerWing {
  points: Pt[];
  tip: Pt;
}

export interface EyelinerGeometry {
  band: Pt[];
  wing: EyelinerWing | null;
  blur: number;
}

export interface BrowGeometryOptions {
  style: BrowGeometryStyle;
  thickness: number;
  browGap: number;
  faceCenterX: number;
}

export interface LashGeometryOptions {
  style: LashGeometryStyle;
  intensity: number;
  length: number;
  curl: number;
}

export interface MakeupCurve {
  base: Pt;
  control: Pt;
  tip: Pt;
  length: number;
...
const magnitude = (point: Pt) => Math.hypot(point.x, point.y) || 1;

const normalize = (point: Pt): Pt => {
  const length = magnitude(point);
  return { x: point.x / length, y: point.y / length };
};

const add = (point: Pt, vector: Pt, amount: number): Pt => ({
  x: point.x + vector.x * amount,
  y: point.y + vector.y * amount
});

const tangentAt = (points: Pt[], index: number): Pt => {
...
const upwardNormalAt = (points: Pt[], index: number): Pt => {
...
export const mapNormalizedPoint = (point: Pt, transform: CoverTransform): Pt => ({
  x: transform.offsetX + point.x * transform.renderedWidth,
  y: transform.offsetY + point.y * transform.renderedHeight
});

export const mapNormalizedPoints = (points: Pt[], transform: CoverTransform): Pt[] =>
  points.map((point) => mapNormalizedPoint(point, transform));

const linerWidthProfile = (style: EyelinerGeometryStyle, t: number) => {
  if (style === 'tightline') return 0.2 + t * 0.2;
  if (style === 'smoky' || style === 'bold') return 0.45 + t * 0.95;
  if (style === 'fox') return 0.12 + t * 0.62;
  if (style === 'puppy') return 0.16 + t * 0.66;
  if (style === 'wing') return 0.14 + t * 0.78;
  return 0.16 + t * 0.52;
};
...
const wingDirection = (
  ...
  let vertical: number;
  if (style === 'puppy') vertical = 0.13 - lift * 0.08;
  else if (style === 'fox') vertical = -(0.14 + (lift + 1) * 0.12);
  else if (style === 'smoky' || style === 'bold') vertical = -(0.12 + (lift + 1) * 0.1);
  else vertical = -(0.2 + (lift + 1) * 0.16);
  return normalize({ x: outward.x, y: outward.y + vertical });
};

export const buildEyelinerGeometry = (
  eye: EyeGeo,
  options: EyelinerGeometryOptions
): EyelinerGeometry => {
  ...
  const baseThickness =
    eye.h * (0.025 + thickness * 0.085) * (0.55 + intensity * 0.65) * styleScale;
  const top = upper.map((point, index) => {
    const t = upper.length === 1 ? 0 : index / (upper.length - 1);
    return add(
      point,
      upwardNormalAt(upper, index),
      baseThickness * linerWidthProfile(style, t)
    );
  });
...
const browArchOffset = (
  ...
  if (style === 'soft-arch') return -browGap * 0.1 * softPeak;
  if (style === 'defined-arch') return -browGap * 0.17 * definedPeak + browGap * 0.025 * t;
  if (style === 'lifted') return -browGap * 0.14 * t;
  return 0;
};

export const shapeBrowRegion = (
  brow: Pt[],
  options: BrowGeometryOptions
): Pt[] => {
...
const lashLengthFactor = (style: LashGeometryStyle, t: number, index: number) => {
  if (style === 'cat') return 0.55 + t * 0.85;
  if (style === 'doll') return 0.58 + Math.sin(Math.PI * t) * 0.88;
  if (style === 'wispy') return (index % 2 === 0 ? 1.12 : 0.72) * (0.74 + t * 0.28);
  return 0.72 + t * 0.3;
};

export const buildLashCurves = (
  eye: EyeGeo,
  options: LashGeometryOptions
): MakeupCurve[] => {
  const upper = eye.upper.length >= 2 ? eye.upper : [eye.inner, eye.outer];
  const intensity = clamp(options.intensity);
...
export const buildBrowHairStrokes = (
  brow: Pt[],
  faceCenterX: number,
  intensity: number
): MakeupCurve[] => {
  if (brow.length < 4) return [];
