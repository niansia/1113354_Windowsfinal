...
import { SIGNAL_TRANSLATIONS } from '../signal/signalText';
import { NEURO_TRANSLATIONS } from '../neuro/neuroText';
import { NOTEBOOK_TRANSLATIONS } from '../notebook/notebookText';
import { LEGAL_TRANSLATIONS } from '../legal/legalText';
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  const t = useCallback(
    (source: string): string => {
      ...
      const entry = SETTINGS_TRANSLATIONS[source] ?? ASSISTANT_TRANSLATIONS[source] ?? ENGINEERING_MATH_TRANSLATIONS[source] ?? DEVELOPMENT_LAB_TRANSLATIONS[source] ?? SPORTS_TRANSLATIONS[source] ?? MEDICAL_TRANSLATIONS[source] ?? SIGNAL_TRANSLATIONS[source] ?? NEURO_TRANSLATIONS[source] ?? NOTEBOOK_TRANSLATIONS[source] ?? LEGAL_TRANSLATIONS[source] ?? STYLE_TRANSLATIONS[source] ?? FEATURE_TRANSLATIONS[source] ?? TRANSLATIONS[source];
