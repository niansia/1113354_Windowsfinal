...
const hasTwoTerminals = (component: CircuitComponent) => component.kind !== 'ground';

const effectiveResistance = (component: CircuitComponent, blownFuses: Set<string>) => {
...
const solveLinearSystem = (matrix: number[][], rhs: number[]) => {
  ...
  for (let column = 0; column < size; column += 1) {
    ...
    for (let col = column; col <= size; col += 1) a[column][col] /= pivotValue;
...
export const simulateCircuit = (document: CircuitDocument): CircuitSimulationResult => {
  const diagnostics: CircuitDiagnostic[] = [];
  const emptyResult = (code: CircuitDiagnostic['code'], message: string): CircuitSimulationResult => ({
    ok: false,
    nodeVoltages: {},
    components: {},
  ...
  if (!document.components.length) return emptyResult('EMPTY_CIRCUIT', '請先放置元件再執行模擬。');

  const ground = document.components.find((component) => component.kind === 'ground');
  if (!ground) return emptyResult('GROUND_REQUIRED', '請先新增並連接接地參考點再執行模擬。');

  const dsu = new DisjointSet();
  for (const component of document.components) {
    dsu.add(terminalId(component.id, 'a'));
    if (hasTwoTerminals(component)) dsu.add(terminalId(component.id, 'b'));
  }
  for (const wire of document.wires) dsu.union(wire.from, wire.to);

  const groundRoot = dsu.find(terminalId(ground.id, 'a'));
  const roots = new Set<string>();
  for (const component of document.components) {
    roots.add(dsu.find(terminalId(component.id, 'a')));
    if (hasTwoTerminals(component)) roots.add(dsu.find(terminalId(component.id, 'b')));
  }

  const nodeRoots = [...roots].filter((root) => root !== groundRoot);
  const nodeIndex = new Map(nodeRoots.map((root, index) => [root, index]));
  const voltageSources = document.components.filter(
    (component) => component.kind === 'battery' || component.kind === 'voltage-source'
  );
  const sourceIndex = new Map(voltageSources.map((component, index) => [component.id, index]));
  const contexts: SolveContext[] = document.components.map((component) => ({
    component,
    nodeA: nodeIndex.get(dsu.find(terminalId(component.id, 'a'))) ?? null,
    nodeB: hasTwoTerminals(component)
      ? nodeIndex.get(dsu.find(terminalId(component.id, 'b'))) ?? null
      : null
  }));

  const ledActive = new Map<string, boolean>();
  const blownFuses = new Set<string>();
  let solution: number[] | null = null;

  const nodeVoltage = (node: number | null) => node === null ? 0 : (solution?.[node] ?? 0);
  ...
  const runSolve = () => {
    ...
    const stampConductance = (nodeA: number | null, nodeB: number | null, conductance: number) => {
      if (nodeA !== null) matrix[nodeA][nodeA] += conductance;
      if (nodeB !== null) matrix[nodeB][nodeB] += conductance;
    ...
    const stampCurrent = (nodeA: number | null, nodeB: number | null, current: number) => {
      if (nodeA !== null) rhs[nodeA] -= current;
      if (nodeB !== null) rhs[nodeB] += current;
    };
  ...
  for (let iteration = 0; iteration < 16; iteration += 1) {
    ...
    let changed = false;
    for (const context of contexts) {
      if (context.component.kind !== 'led') continue;
  ...
  if (!solution) return emptyResult('SINGULAR_CIRCUIT', '電路包含浮接節點或互相衝突的理想電源。');

  const calculateReading = (context: SolveContext): ComponentReading => {
    const { component, nodeA, nodeB } = context;
  ...
  for (const component of document.components) {
    if (component.kind !== 'fuse') continue;
  ...
  const nodeVoltages: Record<string, number> = { [groundRoot]: 0 };
