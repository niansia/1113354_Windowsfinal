...
export interface CircuitPartDefinition {
  kind: CircuitComponentKind;
  name: string;
  symbol: string;
  description: string;
  group: '電源' | '被動元件' | '控制' | '量測' | '參考';
  color: string;
  defaults: Partial<CircuitComponent>;
}

export const CIRCUIT_PARTS: CircuitPartDefinition[] = [
...
export const getPartDefinition = (kind: CircuitComponentKind) =>
  CIRCUIT_PARTS.find((part) => part.kind === kind)!;

const nextLabel = (kind: CircuitComponentKind, index: number) => {
  const prefix: Record<CircuitComponentKind, string> = {
    battery: 'BAT',
    'voltage-source': 'V',
    'current-source': 'I',
    resistor: 'R',
...
export const createCircuitComponent = (
  ...
  const definition = getPartDefinition(kind);
  return {
...
export const rotateCircuitComponent = (rotation: CircuitRotation): CircuitRotation =>
  ((rotation + 90) % 360) as CircuitRotation;

export const parseCircuitDocument = (text: string): CircuitDocument => {
  const parsed = JSON.parse(text) as Partial<CircuitDocument>;
  if (
    parsed.version !== 1 ||
  ...
  const validKinds = new Set(CIRCUIT_PARTS.map((part) => part.kind));
...
export const formatEngineering = (value: number, unit: string) => {
