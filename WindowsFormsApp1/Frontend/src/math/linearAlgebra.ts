...
const validateMatrix = (matrix: Matrix, allowEmpty = false): { rows: number; columns: number } => {
...
export const parseMatrix = (source: string): Matrix => {
  const rows = source
    .trim()
    .split(/[;\n]+/)
    .map((row) => row.trim())
...
export const matrixAdd = (left: Matrix, right: Matrix): Matrix => {
  const a = validateMatrix(left);
  const b = validateMatrix(right);
  if (a.rows !== b.rows || a.columns !== b.columns) throw new Error('DIMENSION_MISMATCH');
...
export const matrixMultiply = (left: Matrix, right: Matrix): Matrix => {
  const a = validateMatrix(left);
  const b = validateMatrix(right);
  if (a.columns !== b.rows) throw new Error('DIMENSION_MISMATCH');
  return Array.from({ length: a.rows }, (_, row) =>
    Array.from({ length: b.columns }, (_, column) =>
      left[row].reduce((sum, value, index) => sum + value * right[index][column], 0)
    )
  );
};

export const reducedRowEchelon = (matrix: Matrix): Matrix => {
  const { rows, columns } = validateMatrix(matrix);
  ...
  for (let column = 0; column < columns && pivotRow < rows; column += 1) {
    ...
    for (let row = pivotRow + 1; row < rows; row += 1) {
      if (Math.abs(result[row][column]) > Math.abs(result[bestRow][column])) bestRow = row;
    }
    if (Math.abs(result[bestRow][column]) < 1e-12) continue;
...
export const determinant = (matrix: Matrix): number => {
  const { rows, columns } = validateMatrix(matrix);
  if (rows !== columns) throw new Error('SQUARE_MATRIX_REQUIRED');
...
export const inverse = (matrix: Matrix): Matrix => {
  ...
  const reduced = reducedRowEchelon(augmented);
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      const expected = row === column ? 1 : 0;
      if (Math.abs(reduced[row][column] - expected) > 1e-8) throw new Error('SINGULAR_MATRIX');
    }
  }
  return reduced.map((row) => row.slice(columns).map(clean));
};

export const solveLinearSystem = (coefficients: Matrix, constants: number[]): number[] => {
  ...
  const reduced = reducedRowEchelon(coefficients.map((row, index) => [...row, constants[index]]));
  for (let row = 0; row < rows; row += 1) {
    if (Math.abs(reduced[row][row] - 1) > 1e-8) throw new Error('SINGULAR_MATRIX');
  }
  return reduced.map((row) => clean(row[columns]));
};

export const formatMatrix = (matrix: Matrix, precision = 6): string =>
  matrix
