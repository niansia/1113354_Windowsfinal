...
exports.SIGNAL_TRANSLATIONS = void 0;
...
exports.SIGNAL_TRANSLATIONS = {
    ...
    '輸入訊息、選擇通道，立即看見封包、延遲與暫存器變化。': entry('输入消息、选择通道，立即看见封包、延迟与寄存器变化。', 'Enter a message, choose a channel, and see packets, latency, and register changes instantly.', 'メッセージとチャネルを選ぶと、パケット、遅延、レジスタ変化が即座に見えます。', '메시지와 채널을 선택하면 패킷, 지연, 레지스터 변화를 즉시 볼 수 있습니다.'),
    ...
    '資料在這裡先被切成位元與訊框，讓錯誤檢查有明確邊界。': entry('数据在这里先被切成位与帧，让错误检查有明确边界。', 'Data is split into bits and frames first, giving error checks clear boundaries.', 'データをビットとフレームに分け、誤り検査の境界を明確にします。', '데이터를 비트와 프레임으로 나누어 오류 검사의 경계를 명확히 합니다.'),
    '處理器追蹤顯示長度、校驗碼與同位元如何進入暫存器。': entry('处理器追踪显示长度、校验码与校验位如何进入寄存器。', 'The processor trace shows how length, checksum, and parity enter registers.', 'プロセッサトレースは長さ、チェックサム、パリティがレジスタへ入る流れを示します。', '프로세서 추적은 길이, 체크섬, 패리티가 레지스터에 들어가는 과정을 보여줍니다.'),
    '物理通道把距離、波速、頻率與雜訊轉成延遲與可靠度。': entry('物理通道把距离、波速、频率与噪声转换成延迟与可靠度。', 'The physical channel turns distance, wave speed, frequency, and noise into latency and reliability.', '物理チャネルは距離、波速、周波数、ノイズを遅延と信頼性に変換します。', '물리 채널은 거리, 파동 속도, 주파수, 잡음을 지연과 신뢰도로 변환합니다.'),
