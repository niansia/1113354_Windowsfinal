...
const flag = (id, label, value, level, explanation, sourceId = 'medlineplus-vitals') => ({ id, label, value, level, explanation, sourceId });
const formatC = (value) => `${value.toFixed(1)} °C`;
const formatBpm = (value) => `${Math.round(value)} /min`;
const formatPercent = (value) => `${Math.round(value)}%`;
const maxLevel = (flags) => flags.reduce((level, item) => LEVEL_WEIGHT[item.level] > LEVEL_WEIGHT[level] ? item.level : level, 'steady');
const summaryFor = (level) => {
    if (level === 'urgent')
        return '目前輸入包含需要立即注意的警示值；若伴隨胸痛、呼吸困難、意識改變或單側無力，請儘快尋求專業醫療協助並立即尋求緊急協助。';
    if (level === 'review')
        return '目前有數值建議與醫療人員討論；請搭配症狀、病史、用藥與量測方式一起判斷。';
    if (level === 'watch')
        return '目前有數值值得持續觀察；建議在相同條件下重新量測並記錄變化。';
    return '目前輸入的生命徵象落在一般成人常見範圍內，仍需搭配症狀與個人病史判斷。';
};
const evaluateVitals = (input) => {
    const flags = [];
