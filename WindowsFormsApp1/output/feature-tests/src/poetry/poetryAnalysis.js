...
exports.analyzePoem = analyzePoem;
const moodFor = (poem) => {
    const text = [...poem.themes, ...poem.imagery].join('');
    if (/家國|戰亂|改革|戰事/.test(text))
        return '沉雄而帶有時代重量';
    if (/離愁|思鄉|懷人|漂泊|羈旅/.test(text))
        return '清寂含愁，情感由景物緩緩展開';
    if (/春日|青春|遊賞/.test(text))
        return '明快清新，帶有流動的生命感';
    if (/禪意|山水|田園/.test(text))
        return '澄澈疏朗，留有安靜而深遠的餘韻';
