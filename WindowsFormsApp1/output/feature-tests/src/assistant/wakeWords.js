...
exports.sanitizeVoiceCommand = sanitizeVoiceCommand;
const WAKE_PATTERNS = [
...
function collapseRepeats(text) {
    ...
    for (const phrase of phrases) {
        ...
        if (!previous || previous.toLowerCase() !== phrase.toLowerCase())
            deduped.push(phrase);
    }
    ...
    if (deduped.length === phrases.length)
        return out;
...
function sanitizeVoiceCommand(input) {
