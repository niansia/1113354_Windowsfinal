...
const hasAny = (text, words) => words.some((w) => w && text.includes(w));
...
function parseSetting(text) {
    ...
    if (hasAny(text, ['音量', '聲音', '声音', 'volume', 'sound', '音量', '볼륨', '소리'])) {
        ...
        if (num !== undefined)
            return { kind: 'setting', setTarget: 'volume', setValue: num };
        if (hasAny(text, ['大聲', '大声', '調大', '调大', '更大', 'louder', 'up', '大きく', '크게', '높여']))
            return { kind: 'setting', setTarget: 'volume', setValue: 'up' };
        if (hasAny(text, ['小聲', '小声', '調小', '调小', '更小', 'quieter', 'down', '小さく', '작게', '낮춰']))
            return { kind: 'setting', setTarget: 'volume', setValue: 'down' };
    }
