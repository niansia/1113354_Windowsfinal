...
exports.composeGroundedAnswer = exports.streamChunks = exports.analyzePrompt = exports.getInferencePlaybackMs = exports.buildInferenceTrace = exports.retrieveLocalKnowledge = exports.calculateReward = exports.stepLiquidNetwork = exports.createAttentionMatrix = exports.tokenizePrompt = exports.LOCAL_KNOWLEDGE = void 0;
...
const analyzePrompt = (prompt, lang) => {
    const lower = prompt.toLocaleLowerCase();
    let intent = 'general';
    for (const candidate of ['compare', 'define', 'reason', 'explain']) {
        if (INTENT_CUES[candidate].some((cue) => lower.includes(cue.toLocaleLowerCase()))) {
            intent = candidate;
            break;
        }
    }
    const focus = (0, exports.tokenizePrompt)(prompt)
...
const streamChunks = (text) => {
    ...
    for (const piece of pieces) {
        if (/^\s+$/u.test(piece) && chunks.length > 0)
...
const composeGroundedAnswer = (prompt, lang, evidence) => {
    ...
    if (top.length === 0)
        return copy.fallback.replace('{q}', prompt);
    const { intent } = (0, exports.analyzePrompt)(prompt, lang);
    const body = top
