...
exports.updateLinearStructure = updateLinearStructure;
exports.insertTreeValue = insertTreeValue;
exports.buildBinarySearchTree = buildBinarySearchTree;
exports.searchTree = searchTree;
exports.removeTreeValue = removeTreeValue;
exports.inOrderTree = inOrderTree;
...
exports.runTraversalTrace = runTraversalTrace;
const cloneMetrics = (metrics) => ({ ...metrics });
const normalizeValue = (value, action) => {
...
function parseNumberList(input) {
    ...
    if (values.length > 40)
        throw new Error('Use 40 numbers or fewer for a readable trace.');
    return values;
}
function updateLinearStructure(kind, values, action, value) {
    const next = [...values];
    if (kind === 'stack') {
        if (action === 'push') {
    ...
    if (kind === 'linked-list') {
        ...
        if (action === 'remove') {
            ...
            if (index < 0)
                throw new Error(`Node ${item} was not found.`);
            next.splice(index, 1);
...
const treeSize = (root) => root ? 1 + treeSize(root.left) + treeSize(root.right) : 0;
function insertTreeValue(root, value) {
    if (!Number.isFinite(value))
        throw new Error('Tree values must be valid numbers.');
    if (!root) {
        return {
    ...
    if (value === root.value)
        return root;
    if (value < root.value)
        return { ...root, left: insertTreeValue(root.left, value) };
    return { ...root, right: insertTreeValue(root.right, value) };
}
function buildBinarySearchTree(values) {
    return values.reduce((root, value) => insertTreeValue(root, value), null);
}
...
function removeTreeValue(root, value) {
    if (!root)
        return null;
    if (value < root.value)
        return { ...root, left: removeTreeValue(root.left, value) };
    if (value > root.value)
        return { ...root, right: removeTreeValue(root.right, value) };
    if (!root.left)
        return root.right;
    if (!root.right)
        return root.left;
...
const makeFrame = (values, metrics, message, pseudocodeLine, activeIndices = [], settledIndices = [], phase = 'ready', auxiliaryValues, range) => ({
    values: [...values],
...
function runSortTrace(algorithm, input) {
    ...
    const values = [...input];
    const frames = [];
    const metrics = { comparisons: 0, writes: 0, visits: 0 };
...
function runSearchTrace(algorithm, input, target) {
    ...
    if (!Number.isFinite(target))
        throw new Error('Enter a valid search target.');
...
function parseGraph(input) {
    const chunks = input
        ...
        .filter(Boolean);
    if (!chunks.length)
        throw new Error('Enter at least one graph edge.');
    const adjacency = {};
    const edges = [];
    const nodes = new Set();
    for (const chunk of chunks) {
        ...
        if (!match)
            throw new Error(`Invalid graph edge "${chunk}". Use A-B format.`);
        const from = match[1];
        const to = match[2];
        if (from === to)
            throw new Error('Self-loop edges are not used in this lab.');
        nodes.add(from);
        nodes.add(to);
        adjacency[from] ??= [];
        adjacency[to] ??= [];
...
const makeGraphFrame = (metrics, message, pseudocodeLine, activeNodes, visitedNodes, phase = 'visit') => ({
    values: [],
    activeIndices: [],
    settledIndices: [],
    activeNodes,
...
function runTraversalTrace(algorithm, graph, startNode) {
