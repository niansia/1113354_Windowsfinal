...
exports.localizeDevelopmentLabMessage = localizeDevelopmentLabMessage;
const entry = (en, zhCN, ja, ko) => ({
    en,
    ...(zhCN ? { 'zh-CN': zhCN } : {}),
    ...(ja ? { ja } : {}),
    ...(ko ? { ko } : {})
...
exports.DEVELOPMENT_LAB_TRANSLATIONS = Object.fromEntries(Object.entries(ENGLISH_TEXT).map(([source, en]) => [source, entry(en)]));
Object.assign(exports.DEVELOPMENT_LAB_TRANSLATIONS, {
...
const pick = (lang, values) => {
    if (lang === 'zh-TW')
        return values['zh-TW'];
    if (lang === 'zh-CN')
        return values['zh-CN'] ?? values['zh-TW'];
    if (lang === 'ja')
        return values.ja ?? values['zh-TW'];
    if (lang === 'ko')
        return values.ko ?? values['zh-TW'];
    return values['zh-TW'];
};
const exactMessages = {
...
const replace = (template, values) => values.reduce((output, value, index) => output.replaceAll(`{${index}}`, value), template);
const rules = [
...
function localizeDevelopmentLabMessage(message, lang) {
    if (lang === 'en')
        return message;
    const exact = exactMessages[message];
    if (exact)
        return pick(lang, exact);
    for (const rule of rules) {
        const match = message.match(rule.pattern);
        if (match)
            return replace(pick(lang, rule.templates), match.slice(1));
    }
    return pick(lang, {
        'zh-TW': '狀態已更新。',
        'zh-CN': '状态已更新。',
