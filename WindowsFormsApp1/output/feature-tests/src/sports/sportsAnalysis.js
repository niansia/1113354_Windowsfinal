...
exports.buildSquadInsight = buildSquadInsight;
const fmtAge = (summary) => summary && summary.avgAge != null ? summary.avgAge.toFixed(1) : '—';
const youngerVerdict = (home, away, p, lang) => {
    if (home.avgAge == null || away.avgAge == null)
        return '';
...
const attackVerdict = (home, away, p, lang) => {
...
function buildSquadInsight(lang, params) {
    ...
    if (home && away) {
        ...
        if (younger)
            verdicts.push(younger);
        if (attack)
            verdicts.push(attack);
    }
