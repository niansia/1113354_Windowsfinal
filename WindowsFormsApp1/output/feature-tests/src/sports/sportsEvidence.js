...
exports.buildPredictionEvidence = buildPredictionEvidence;
exports.applyPredictionEvidence = applyPredictionEvidence;
const sportsStrength_js_1 = require("./sportsStrength.js");
const clamp = (value, minimum, maximum) => Math.min(maximum, Math.max(minimum, Number.isFinite(value) ? value : 0));
const signed = (difference, scale, limit) => clamp(difference * scale, -limit, limit);
const recentScore = (games) => {
    if (!games.length)
        return null;
...
const resultLabel = (score) => score === null ? '—' : `${Math.round(score * 100)}%`;
const standingFor = (detail, teamId) => detail?.standings.find((row) => row.teamId === teamId);
const recentFor = (detail, teamId) => detail?.teams.find((team) => team.teamId === teamId)?.recentGames ?? [];
const h2hScore = (games, homeId, awayId) => {
    if (!games.length)
        return null;
    let home = 0;
    let away = 0;
    ...
    for (const game of games) {
        if (game.teamId !== homeId && game.teamId !== awayId)
            continue;
        samples += 1;
...
function buildPredictionEvidence(options) {
    ...
    if (typeof home.rating === 'number' && typeof away.rating === 'number') {
        ...
        miss(1.1);
    const homeRecord = (0, sportsStrength_js_1.parseTeamRecord)(home.record);
    const awayRecord = (0, sportsStrength_js_1.parseTeamRecord)(away.record);
    ...
    const formSignal = clamp(factors
        .filter((factor) => factor.id === 'recent-form' || factor.id === 'record')
        .reduce((sum, factor) => sum + factor.impact * factor.confidence, 0) / 5, -18, 18);
    return {
        factors,
...
function applyPredictionEvidence(input, evidence) {
    return {
        ...input,
        home: {
            ...input.home,
