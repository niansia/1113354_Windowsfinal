...
const evidenceNarrative = (evidence, lang) => {
    if (!evidence?.factors.length)
        return '';
    const factors = [...evidence.factors]
        .sort((left, right) => Math.abs(right.impact * right.confidence) - Math.abs(left.impact * left.confidence))
        .slice(0, 3)
        .map((factor) => {
    ...
    if (lang === 'en')
        return ` Key factors (${coverage}% data coverage): ${factors.join('; ')}.`;
    if (lang === 'ja')
        return ` 主な要因（データ網羅率 ${coverage}%）：${factors.join('；')}。`;
    if (lang === 'ko')
        return ` 주요 요인(데이터 범위 ${coverage}%): ${factors.join('; ')}.`;
    if (lang === 'zh-CN')
        return ` 关键因素（数据覆盖率 ${coverage}%）：${factors.join('；')}。`;
    return ` 關鍵因素（資料覆蓋率 ${coverage}%）：${factors.join('；')}。`;
};
async function generateSportsReport(options) {
    const local = `${(0, sportsSimulation_js_1.buildLocalSportsReport)(options.input, options.result, options.lang)}${evidenceNarrative(options.evidence, options.lang)}`;
    if (!options.useAI)
