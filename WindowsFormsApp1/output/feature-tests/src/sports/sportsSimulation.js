...
exports.buildLocalSportsReport = buildLocalSportsReport;
const clamp = (value, minimum, maximum) => Math.min(maximum, Math.max(minimum, Number.isFinite(value) ? value : minimum));
function createPredictionInput(options) {
    const side = (name, rating, offense, defense, form) => ({
...
const randomPoisson = (lambda, random) => {
...
const sideStrength = (side) => side.rating + (side.form - 50) * 4 + (side.offense - side.defense) * 1.5;
const logisticProbability = (difference) => 1 / (1 + 10 ** (-difference / 400));
function simulateMatch(input) {
    ...
    for (let index = 0; index < input.iterations; index += 1) {
        ...
        if (input.model === 'goals') {
            const homeExpected = clamp(1.4 * Math.exp(adjustedDifference / 1000) *
                (0.8 + input.home.offense / 250) *
                (1.2 - input.away.defense / 500), 0.2, 4.8);
            const awayExpected = clamp(1.12 * Math.exp(-adjustedDifference / 1100) *
                (0.8 + input.away.offense / 250) *
                (1.2 - input.home.defense / 500), 0.15, 4.5);
            ...
            if (homeScore === awayScore) {
                if (random() < logisticProbability(adjustedDifference))
                    homeScore += 1;
                else
                    awayScore += 1;
            }
...
function buildLocalSportsReport(input, result, lang) {
