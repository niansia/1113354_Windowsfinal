...
exports.ROSTER_SPORTS = void 0;
exports.summarizeSquad = summarizeSquad;
exports.normalizeEspnRoster = normalizeEspnRoster;
...
const groupForPosition = (abbreviation) => {
    const code = abbreviation.toUpperCase();
    if (/^(G|GK)$/.test(code))
        return 'GK';
    if (/^(D|DF|CB|LB|RB|LWB|RWB|DEF)$/.test(code))
        return 'DEF';
    if (/^(M|MF|CM|DM|AM|LM|RM|MID)$/.test(code))
        return 'MID';
    if (/^(F|FW|ST|CF|LW|RW|SS|FWD|W)$/.test(code))
        return 'FWD';
    return 'OTHER';
};
const normalizeAthlete = (value) => {
...
const flattenAthletes = (athletes) => {
...
function summarizeSquad(players) {
    const byGroup = { GK: 0, DEF: 0, MID: 0, FWD: 0, OTHER: 0 };
    let ageSum = 0;
    let ageCount = 0;
    let available = 0;
...
function normalizeEspnRoster(payload, teamId) {
...
const fetchJson = async (fetcher, url, timeoutMs) => {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Roster provider returned HTTP ${response.status}.`);
...
async function loadTeamRoster(options) {
