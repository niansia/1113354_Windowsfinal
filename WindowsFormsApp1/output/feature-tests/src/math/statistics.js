...
exports.zScore = exports.normalCdf = exports.normalPdf = exports.binomialProbability = exports.linearRegression = exports.describeSample = exports.parseNumberList = void 0;
const discreteMath_js_1 = require("./discreteMath.js");
...
const describeSample = (sample) => {
    if (sample.length === 0 || sample.some((value) => !Number.isFinite(value)))
        throw new Error('INVALID_SAMPLE');
...
exports.describeSample = describeSample;
...
const binomialProbability = (n, k, probability) => {
    if (probability < 0 || probability > 1)
        throw new Error('INVALID_PROBABILITY');
    return (0, discreteMath_js_1.combinations)(n, k) * probability ** k * (1 - probability) ** (n - k);
};
exports.binomialProbability = binomialProbability;
const normalPdf = (value, mean = 0, stdDev = 1) => {
    if (!(stdDev > 0))
        throw new Error('INVALID_STANDARD_DEVIATION');
...
const normalCdf = (value, mean = 0, stdDev = 1) => {
    if (!(stdDev > 0))
        throw new Error('INVALID_STANDARD_DEVIATION');
    if (value === mean)
        return 0.5;
